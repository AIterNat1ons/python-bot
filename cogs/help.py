...
class Help(commands.Cog):
    ...
    async def help(self, ctx, number = None):
        ...
        if number == 'mod':
            ...
            if number == 'music':
                musichelpembed = discord.Embed(
                    title = '**Music Commands!**',
                    description = 'WIP'
                )
                musichelpembed.set_footer(text = 'This is only commands for the category \'music\', if you want to see the rest of the categories type in only .help.')
                await ctx.send(embed = musichelpembed)               
